#make repeat nums reflect add speed
#add a pause button, toggle arms
#combine + and - buttons with right click
...
game_states = ({"hue": 0, "hue_step": .0002, "gcd_value": 0, "num_arms": 0, "decay": 0,
                "normal_speed": 5, "new_normal_speed": 5, "normal_length": border-5, "new_normal_length": border-5,

                "running": True, "show_info": True, "show_trail": True, "show_lines":True, "rainbow": True, "show_arms": True, "start": True,
                "add_speed": False, "repeated": False, "reset": False, "random_inputs": False, "pause": False})

repeat_nums, combined_repeat_nums, arm_list, fractions = [], [], [], [] #trail_list
length_list, adjusted_lengths, length_percents, length_ratios = [], [], [], []
speed_list,  adjusted_speeds, speed_percents, speed_ratios, combined_speeds = [], [], [], [], []
text = ""
#endregion setup
...
def reset_pattern_state(game_states):
    ...
    game_states["add_speed"] = game_states["pause"] = False #?
    adjusted_lengths.clear()
    length_percents.clear()
    speed_percents.clear()
    adjusted_speeds.clear()
    for button in button_list:
        if button.button_name not in ("Toggle Info", "Toggle Arms"):
            button.press = False
...
def add_decay(button, game_states):
    game_states["decay"] = (game_states["decay"] + .0001) if button.button_name == "Decay +" else (game_states["decay"] - .0001)

def add_rainbow(button, game_states):
    game_states["hue_step"] = (game_states["hue_step"] + .0002) if button.button_name == "Rainbow +" else (game_states["hue_step"] - .0002)
...
def toggle_color (button, game_states):
    game_states["rainbow"] = not game_states["rainbow"]
    button.press = False if game_states["rainbow"] else True

def toggle_trail (button, game_states):
    game_states["show_trail"] = not game_states["show_trail"]
    button.press = False if game_states["show_trail"] else True

def toggle_info (button, game_states):
    game_states["show_info"] = not game_states["show_info"]
    button.press = False if game_states["show_info"] else True

def toggle_lines(button, game_states):
    game_states["show_lines"] = not game_states["show_lines"]
    button.press = False if game_states["show_lines"] else True

def toggle_arms(button, game_state):
    game_states["show_arms"] = not game_states["show_arms"]
    button.press = False if game_states["show_arms"] else True
...
def add_speeds(button, game_states):
    global adjusted_speeds
    game_states["add_speed"] = not game_states["add_speed"]
    button.press = False if not game_states["add_speed"] else True

def change_speed(button, game_states):
    adjusted_speeds.clear()
    speed_percents.clear()

    game_states["new_normal_speed"] = (game_states["new_normal_speed"] * 1.15 if button.button_name == "Speed +" 
...
def pause_game(button, game_states):
    game_states["pause"] = not game_states["pause"]
    button.press = False if not game_states["pause"] else True

def exit(button, game_states):
    game_states["running"] = False

button_names = (["Reset Pattern", "Reset Trail", "Toggle Friction", "Toggle Trail", "Toggle Color", "Toggle Lines", "Toggle Info", "Speed +", "Speed -", 
                 "Zoom +", "Zoom -", "Decay +", "Decay -", "Exit", "New Pattern", "Random Pattern", "Pause", "Rainbow +", "Rainbow -", "Toggle Arms"]) 
button_functions = ([reset_pattern, reset_trail, add_speeds, toggle_trail, toggle_color, toggle_lines, toggle_info, change_speed, change_speed, 
                    zoom, zoom, add_decay, add_decay, exit, new_pattern, random_pattern, pause_game, add_rainbow, add_rainbow, toggle_arms])
#endregion Buttons
...
class Arm:
    ...
    def update_pos(self, game_states, parent = None):    
        global decay
        self.reset = False
        if parent != None:
            self.x_start = parent.x_end
            self.y_start = parent.y_end
            if game_states["add_speed"]:
    ...
    def display_arms(self):
        pygame.draw.circle(screen, GREY, (self.x_start, self.y_start), self.length, 1)
