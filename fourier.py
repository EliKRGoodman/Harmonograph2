...
game_states = ({"hue": 0, "gcd_value": 0, "num_arms": 0, "decay": 0, "screenshot_timer": 0,
                "hue_step": .0002, "normal_speed": 5, "new_normal_speed": 5, 
                "normal_length": border-5, "new_normal_length": border-5,
                ...
                "pause": False, "back": False, "sound_fx": False, "initialized": False,
                "last_click":"left"})

sound_constants = ({"freq_high": 520, "freq_low": 140, "duration": .2, "volume": .1})
...
def reset_pattern_state(): #used both for reseting pattern and new pattern
...
def reset_pattern(button): #used just for a new pattern
    reset_pattern_state()
...
def new_pattern(button):
    game_states["random_inputs"] = False
    change_pattern()

def random_pattern(button):
...
def toggle_button(button, string, not_default_state):
    game_states[string] = not game_states[string]
    button.press = not_default_state if game_states[string] else not not_default_state

def show_buttons(button):
    toggle_button(button, "show_buttons", False)

def toggle_color (button):
    toggle_button(button, "rainbow", False)

def toggle_trail (button):
    toggle_button(button, "show_trail", False)

def toggle_info (button):
    toggle_button(button, "show_info", False)

def toggle_lines(button):
    toggle_button(button, "show_lines", False)

def toggle_arms(button):
    toggle_button(button, "show_arms", False)

def add_speeds(button):
    toggle_button(button, "add_speed", True)

def pause_game(button):
    toggle_button(button, "pause", True)
...
def toggle_sound(button):
    toggle_button(button, "sound_fx", True) 

def back_button(button):
    game_states["back"] = True

def show_explanation(button):
...
class Arm:
    ...
    def update_pos(self, parent = None): #use self.parent instead
        ...
        if not (0 < self.main_angle < 360) and self.speed != 0:
            self.main_angle = self.main_angle % 360
            global channels
            if game_states["initialized"] and game_states["sound_fx"] and not channels[self.arm_num].get_busy():
                self.color = button_color
                channels[self.arm_num].play(sounds[self.arm_num])
    ...
    def display_trail(self, max_ghost_length):
...
#region title screen
def render_text(text, font_size=36, color=(255, 255, 255)): #ChatGPT
...
def title_screen(): 
    ...
    while(game_states["title"]):
        ...
        for i in range(len(title_arms)):
            title_arms[i].update_pos(title_arms[i-1]) if i > 0 else title_arms[i].update_pos()
...
def build_title_arms():
    ...
    title_arms.extend([title_arm1, title_arm2, title_arm3])
#endregion title screen

#region GameLoop
def get_inputs():
    game_states["num_arms"] = 0
    while game_states["num_arms"] < 1: #semi-validates input against negative numbers
...
def build_arms():
    ...
    repeat_nums = calculate_repeat_nums(speed_list)
    combined_repeat_nums = calculate_repeat_nums(combined_speeds)
    calculate_ratios(speed_percents, speed_ratios)
    calculate_ratios(length_percents, length_ratios)
...
def next_frame():
    ...
    if game_states["screenshot_timer"] != 0:
...
def initialize_sound_stuff():
    ...
    frequencies = calculate_frequencies(game_states["num_arms"], sound_constants["freq_high"], sound_constants["freq_low"])
    pygame.mixer.set_num_channels(game_states["num_arms"])
    sounds = [generate_sound(freq, sound_constants["duration"], sound_constants["volume"]) for freq in frequencies]
    channels = [pygame.mixer.Channel(i) for i in range(game_states["num_arms"])]
    game_states["initialized"] = True

def game_loop(arm_list, adjusted_lengths):
    create_buttons()
    title_screen()

    while game_states["running"]:
        if game_states["start"]:
            initialize_sound_stuff()
        
        next_frame()
        arm_list[-1].display_trail(max_ghost_length)
        display_arms()     
